...
const {
  ParameterException
} = require('./exception/http-exception.js')
const {
  get,
  last,
  set,
...
class LinValidator {
   ...
   * @param {boolean} [parsed=true] 是否转型 默认自动转型
   * @return {*}  转型后的数据
   * @memberof LinValidator
   */
...
class Rule {
   ...
   * @param {string} name 属性需要满足的规则
   * @param {string} msg 不满足规则的提示信息
   * @param {object} params 可选参数，可以对属性进行限制 {min:1}
   * @memberof Rule
   */
